...
第一种最常规的思路就是双指针遍历，时间o(n^2)

最开始想到了排序，后面否决了，因为要的是原始的下标组合

```python
class Solution:
    def twoSum(self, nums: List[int], target: int) -> List[int]:
        # 常规思路：双层遍历，直接暴力，更推荐直接先排序。错误的，不能排序，这样索引会变动
        length = len(nums)
...
![image-20260118142806398](./assets/image-20260118142806398.png)

优化时间的思路：

大概率是拿空间换时间，可以想到哈希表在读取元素的时候时间复杂度为o(1)，也就是说针对当前元素i，只需要哈希查找target-nums[i]在不在哈希表中即可。

所以哈希表的键就是nums[i]，对应的值就是它的下标i
...
class Solution:
    def twoSum(self, nums: List[int], target: int) -> List[int]:
        # 优化：双指针？本质上就是暴力遍历
        # 哈希表，搜索的代价仅为o(1) 但是哈希表的创建有些遗忘了，有个问题是有重复元素
        # a+b = target, b = target-a
        # 时间上的优化都是拿空间换取的
        # 隐含了一个条件，如果有重复元素，就不会只存在一个有效答案了，所以重复元素的下标可以直接返回了，所以还是创建哈希表
        hash_map = dict()
...
执行错误，错在` j = hash_map[target-nums[i]]`这一行上，源于python字典的特性，我在没有复习的情况下想的是找不到元素就返回-1，但是实际上python的字典在找不到元素的时候会抛出`KeyError`异常，所以执行错误了，正确的做法是用in来判断元素是否存在

修正错误之后在case`[1,1,1,1,1,4,1,1,1,1,1,7,1,1,1,1,1]`上解答错误，因为我想当然的把重复元素直接返回了，得到的结果是[0,1]，这是错误的，所以在重复元素之上还需要考虑是否是target的一半，这里也不用考虑target的奇偶了，比如target是7，除以2得到3.5，而元素都是整数，就不用管了。
...
class Solution:
    def twoSum(self, nums: List[int], target: int) -> List[int]:
        ...
        # 时间上的优化都是拿空间换取的
        # 隐含了一个条件，如果有重复元素，就不会只存在一个有效答案了，所以重复元素的下标可以直接返回了，所以还是创建哈希表
        # 一个测试case直接否认了这种做法，还需要判断这个重复元素是否直接是target的一半，如果不是的话，相同元素其实可以抛弃了，或者直接覆盖也是可以的
        hash_map = dict()
        for i in range(len(nums)):
            # if hash_map[nums[i]] == -1:
            if nums[i] not in hash_map or nums[i] * 2 != target:
                hash_map[nums[i]] = i
            else:
                return [hash_map[nums[i]], i]
        for i in range(len(nums)):
...
![image-20260118144056584](./assets/image-20260118144056584.png)

其实题目还隐含的是返回顺序无所谓，所以上面的做法还可以从右到左遍历

暂时想不出来更好的解法了，直接看答案来学习

## Python参考答案

上面的做法实际上是用了两次遍历，不是真正意义上的o(n)，要想做到一次遍历即可实现，需要的是在每次哈希的时候同步找哈希表中是否存在目标的target-当前元素对应的值

为什么呢？因为对于当前元素，左侧的是已知已经哈希存储的，所以对当前元素，先哈希查找目标元素在不在，然后再更新哈希表
...
我的初步想法是字典套字典的方案，因为在python环境中试过了，
...
对于字典，只要键值对内容一致，就是一样的东西，所以初步的想法就是字典嵌套，实现一个类似
...
这样的结构，然后再遍历原始的list，内部嵌套一层for再遍历，比较内部值是否一致，如果一致，则可以填写到一层answer中了。

所以这样看来，先构建双层dict，时间复杂度o(mn)，再来双层遍历，时间复杂度o(n^2)
...
class Solution:
    def groupAnagrams(self, strs: List[str]) -> List[List[str]]:
        # 本质上构造多个哈希表 也就是字典中嵌套字典
        ans = [] # 定义返回的列表
        if len(strs) == 1:
            return [strs]
        hash_map = {}
        for idx, e in enumerate(strs):
            inner_hash = {}
            for i in range(len(e)):
                if e[i] in inner_hash:
                    inner_hash[e[i]] += 1
                else:
                    inner_hash[e[i]] = 1
            hash_map[e] = inner_hash

        for idx, e in enumerate(strs):
            inner_ans = []
            if e in hash_map:
                inner_ans.append(e)
                value = hash_map.pop(e)
                for j in range(idx+1, len(strs)):
                    if strs[j] in hash_map and value == hash_map[strs[j]]:
                        hash_map.pop(strs[j]) # 弹出元素
                        inner_ans.append(strs[j])
            if len(inner_ans):
                ans.append(inner_ans)
        return ans
```

遇到这个case错误：`["",""]`，因为我没有考虑重复元素的问题。

二次提交，在一次的基础上增加一个dict，专门用来记录每个元素出现的次数
...
class Solution:
    def groupAnagrams(self, strs: List[str]) -> List[List[str]]:
        ...
        for idx, e in enumerate(strs):
            inner_ans = []
            if e in hash_map:
                for times in range(0,times_hash_map[e]):
                    inner_ans.append(e)
                value = hash_map.pop(e)
                for j in range(idx+1, len(strs)):
                    if strs[j] in hash_map and value == hash_map[strs[j]]:
...
![image-20260118175217818](./assets/image-20260118175217818.png)

说白了，错因是内部实现inner_ans数组的时候没有考虑重复元素：
            ...
            if e in hash_map:
                ...
                for j in range(idx+1, len(strs)):
                    if strs[j] in hash_map and value == hash_map[strs[j]]:
                        hash_map.pop(strs[j]) # 弹出元素
                        # 这里也要多次添加
                        for times in range(0,times_hash_map[strs[j]]):
                            inner_ans.append(strs[j])
            if len(inner_ans):
                ans.append(inner_ans)
        return ans
...
结果，可以看到只是勉强通过，主要在于后面的二重循环吧，把时间复杂度上升到了n2：

![image-20260118175316797](./assets/image-20260118175316797.png)

## Python参考答案

**如果两个字符串从小到大排序后相等，那么两个字符串就互为字母异位词，否则不是**。【最省时间的还是先对单词排序】
...
还有一个思路是不排序，统计每一个单词中每个字母出现的频次tuple，以tuple作为键，list作为值，最后返回字典的values即可
